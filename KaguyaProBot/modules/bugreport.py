...
from KaguyaProBot import dispatcher, SUPPORT_CHAT_ID
...
def bug(update: Update, context: CallbackContext):
    ...
    if len(to_send.split(" ")) >= 2:
        try:
            ...
            update.effective_message.reply_text("Bug request Submitted successfully, contact @ShinomiyaSupport for escalations.")
...
✮ `/bug <report text>`: Sends a string containing the bot issues directly to Kaguya's support private channel.
...
Reach out to @ShinomiyaSupport for any queries.
