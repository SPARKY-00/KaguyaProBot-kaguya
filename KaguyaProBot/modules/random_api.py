...
from KaguyaProBot import TEMP_DOWNLOAD_DIRECTORY, pbot
...
async def boobs(client, message):
    ...
    a = await message.reply_text("**Muoo! Ecchi!!**")
    await asyncio.sleep(0.5)
    await a.edit("` Sending pic for boobs, Matte-kudasai...`")
    nsfw = requests.get("http://api.oboobs.ru/noise/1").json()[0]["preview"]
    urllib.request.urlretrieve("http://media.oboobs.ru/{}".format(nsfw), pic_loc)
    await client.send_photo(message.chat.id, pic_loc, caption="**Here's the Oppai pic.**")
    os.remove(pic_loc)
    await a.delete()
